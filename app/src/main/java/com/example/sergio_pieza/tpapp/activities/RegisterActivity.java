...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void postDataToSQLite() {
        if (!inputValidation.isInputEditTextFilled(textInputEditTextName, textInputLayoutName, getString(R.string.error_falta_nombre))) {
            return;
        }
        ...
        if (!inputValidation.isInputEditTextFilled(textInputEditTextPassword, textInputLayoutPassword, getString(R.string.error_falta_contrasena))) {
            return;
        }
        if (!inputValidation.isInputEditTextMatches(textInputEditTextPassword, textInputEditTextConfirmPassword,
                textInputLayoutConfirmPassword, getString(R.string.error_contrasena_distinta))) {
            return;
        }
        ...
        if (!databaseHelper.checkUser(textInputEditTextEmail.getText().toString().trim())) {
            ...
            Snackbar.make(nestedScrollView, getString(R.string.registro_existoso), Snackbar.LENGTH_LONG).show();
            emptyInputEditText();


        } else {
            // Snack Bar to show error message that record already exists
            Snackbar.make(nestedScrollView, getString(R.string.error_email_usado), Snackbar.LENGTH_LONG).show();
        }
